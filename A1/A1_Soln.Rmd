...
From the standard deviations, we calculate that the ratio of the largest within-group and biggest within-group variance estimate is $\dfrac{\sigma_{1990}}{\sigma_{2000}} = \dfrac{0.480}{0.203} = 2.365 < 3$. According to the rule of thumb from Dean and Voss, the assumption for equality of variances is satisfied.
...
<!-- TODO: is the linear model the same model as the ANOVA model? -->
...
knitr::kable(cbind(newData[, -3], 1000 * expSmokePred / (1 + expSmokePred)),
             digits = 1)
```
<!-- TODO: are these logodds and odds? -->
